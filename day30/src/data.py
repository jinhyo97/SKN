...
class TipsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 데이터프레임에서 'tip' 열을 제외한 특성값을 가져와서 NumPy 배열로 변환한 뒤, PyTorch 텐서로 변환
        X = torch.from_numpy(self.data.iloc[idx].drop('tip').values).float()
        
        # 'tip' 열의 값을 텐서로 변환하여 레이블을 생성
        y = torch.Tensor([self.data.iloc[idx].tip]).float()
...
class TipsDataModule(L.LightningDataModule):
